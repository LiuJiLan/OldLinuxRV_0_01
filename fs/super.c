...
static int set_bit(int bitnr, unsigned long *addr) {
    unsigned long mask = 1UL << (bitnr % (sizeof(unsigned long) * 8));
    int res = (*addr & mask) != 0; // Test if the bit is already set
    *addr |= mask;                // Set the bit
    return res;                   // Return the original state of the bit
}
