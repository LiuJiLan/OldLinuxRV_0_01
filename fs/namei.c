...
static int match(int len, const char *name, struct dir_entry *de) {
    ...
    // Compare strings up to 'len' characters
    return strncmp(name, de->name, len) == 0 ? 1 : 0;
}
...
int sys_mkdir(struct pt_regs * regs)
{
...
int sys_rmdir(struct pt_regs * regs)
{
    const char * name = regs->a0;
...
int sys_unlink(struct pt_regs * regs)
{
    const char * name = regs->a0;
