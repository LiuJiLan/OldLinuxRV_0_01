...
extern void hd_interrupt(struct pt_regs *);
...
//	outb(cmd,++port);
    ...
    // block = cyl * hd_info.head + head
    // sector = hd_info.sect * block + sect
    // 又因为 sect 加过1, 所以最后还要-1
    sector = disk_cfg->geometry.sectors * (cyl * disk_cfg->geometry.heads + head) + sect - 1;
    virtio_cmd(cmd,  sector);
}
...
//		return;
//	}
    memcpy(this_request->bh->b_data+512*(this_request->nsector&1),
           virt_buffer, 512);
    VIRT_ACK;
...
void hd_init(void)
{
    ...
    virtio_mmio_init();
    hd_info[0].cyl =  disk_cfg->geometry.cylinders;
    hd_info[0].head = disk_cfg->geometry.heads;
    hd_info[0].sect = disk_cfg->geometry.sectors;
